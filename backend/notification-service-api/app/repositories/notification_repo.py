...
from app.schemas.base import AppBasePagingRes
...
async def get_by_id(id_str: str, user_id: str) -> dict | None:
    ...
    if doc:
        return to_notification_res(doc, user_id)
    return None


async def get_by_filter(params: GetNotificationReq, tenant_id: str, user_id: str):
...
async def get_all_info_by_id(id_str: str) -> dict | None:
    uid = uuid.UUID(id_str)
    bson_id = Binary(uid.bytes, UUID_SUBTYPE)
