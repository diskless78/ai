...
def transform_timestamp_to_find_previous(start, end, previous_type):
    ...
    if previous_type == "yesterday":
        return {"start": int(start) - 86400, "end": int(start) - 1}
...
def calculate_percent(now, pre):
    ...
    for i in range(len(pre)):
        ...
        if pre[i - 1] == 0:
            ...
            increase = False
            percentage = ((int(pre[i - 1]) - int(now[i])
                           ) / int(pre[i - 1])) * 100
        else:
            increase = True
            percentage = ((int(now[i]) - int(pre[i - 1])
                           ) / int(pre[i - 1])) * 100
...
def count_people_v2(date_start, date_end, date_start_previous,
    ...
    get_percent_increase = calculate_percent(total, totalPrevious)
...
def count_pc_day_to_day_v2(date_start, date_end,
                           date_start_previous, date_end_previous, list_box_id, zone_location):
    """
        :param date_start:
        :param date_end:
        :param date_start_previous:
...
def count_customer(start, end, start_previous, end_previous, list_box_id, is_new_customer):
    ...
    get_percent_increase = calculate_percent(result, result_pre)
    percentage, increase = get_percent_increase["percentage"], get_percent_increase["increase"]
    static.update({
        "Total": result,
...
def _extracted_from_count_customer_3(arg0, arg1, list_box_id, is_new_customer):
    """
        :param arg0:
        :param arg1:
        :param list_box_id:
    ...
    query_customer = {
        "$and":
            [
                {
                    "timestamp": {"$gte": int(arg0), "$lte": int(arg1)}
                },
                {
                    "boxID": {"$in": list_box_id}
                },
    ...
    for data in find_customer:
        for record in data["listRecord"]:
            day_number_in_week = get_day_number_in_week(arg0)
            if (
                record["newCustomer"] == is_new_customer
                # and int(arg0) <= data["timestamp"] < int(arg1)
            ):
                if day_number_in_week == 0:
                    emp_listt0.append(data)
...
def count_customer_day_to_day_v2(date_start, date_end, date_start_previous, date_end_previous,
    ...
    get_static_data_customer = count_customer(date_start,
                                              ...
                                              list_box_id,
                                              is_new_customer)
    data_now = get_static_data_customer["Total"]
    data_pre = get_static_data_customer["TotalPrevious"]

    for i in range(len(data_pre)):
        if i >= len(data_pre):
            break
        list_data_previous_float[i] = data_pre[i]

    for index in range(len(data_now)):
        if index >= len(data_now):
            break
        list_data_float[index] = data_now[index]
    ...
    for elem in range(len(list_data)):
        if elem >= len(list_data):
            break
...
def get_rate_day_to_day(first_zone, second_zone, first_zone_previous,
                        second_zone_previous, name):  # sourcery no-metrics
    """
        :param first_zone:
        :param second_zone:
    ...
    for elem in range(len(first_zone["data"])):
        if elem >= len(first_zone["data"]):
            break
        ...
        if str(second_zone_previous["data"][elem - 1]) == "0.00":
            rate_previous = 100000001.0
        else:
            try:
                x = float(first_zone_previous["data"][elem - 1])
                y = float(second_zone_previous["data"][elem - 1])
                rate_previous = (x / y) * 100
            except BaseException as error:
                capture_exception(error)
        if int(rate_data) == 100000001 and int(rate_previous) != 100000001:
            rate_per = -100
        elif int(rate_data) != 100000001 and int(rate_previous) == 100000001:
            rate_per = 0
        else:
            rate_per = rate_data - rate_previous

        listRatePercentage.append(str("{:.2f}".format(rate_per)))
    rate.update({"data": listRate})
    ratePercentage.update({"data": listRatePercentage})
    return rate, ratePercentage


def response_get_static_by_week_yesterday(date_start, date_end, type_previous,
                                          date_end_timestamp,
                                          list_box_id):
    """
        :param date_start:
    ...
    gateCountByFuckingWeek = count_pc_day_to_day_v2(
        date_start, date_end, start_previous,
        end_previous, list_box_id, "gate")
    ...
    shopCountByFuckingWeek = count_pc_day_to_day_v2(
        date_start, date_end, start_previous, end_previous, list_box_id, "shop")
    ...
    # get percentage
    rate1, rate1_percentage = get_rate_day_to_day(peopleCountInGateFromWeek,
                                                  peopleCountInWayFromWeek,
    ...
    get_old_cus = count_customer_day_to_day_v2(
        date_start, date_end, start_previous, end_previous, list_box_id, False)
    ...
    get_new_customer = count_customer_day_to_day_v2(
        date_start, date_end, start_previous, end_previous, list_box_id, True)
