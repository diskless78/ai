...
#include "../cuda/nms_rotated.h"

using namespace nvinfer1;

#define PLUGIN_NAME "NMSRotated"
...
namespace rapid
{
  ...
  class NMSRotatedPlugin : public IPluginV2DynamicExt
  {
    float _nms_thresh;
  ...
  public:
    NMSRotatedPlugin(float nms_thresh, int detections_per_im)
    ...
    NMSRotatedPlugin(float nms_thresh, int detections_per_im, size_t count)
    ...
    NMSRotatedPlugin(void const *data, size_t length)
    {
                                  ...
                                  int nbInputs, IExprBuilder &exprBuilder) noexcept override
    {
      ...
      if (outputIndex == 1)
      {
        output.d[1] = exprBuilder.constant(_detections_per_im * 5);
      }
      else
      {
                            ...
                            int nbInputs, const PluginTensorDesc *outputs, int nbOutputs) const noexcept override
    {
      if (size < 0)
      {
        size = cuda::nms_rotated(inputs->dims.d[0], nullptr, nullptr, _count,
    ...
    {
      return cuda::nms_rotated(inputDesc->dims.d[0], inputs, outputs, _count,
                               _detections_per_im, _nms_thresh,
                         ...
                         const DynamicPluginTensorDesc *out, int nbOutputs) noexcept override
    {
      assert(nbInputs == 2);
      assert(nbOutputs == 2);
      assert(in[1].desc.dims.d[1] == in[0].desc.dims.d[1] * 5);
      _count = in[0].desc.dims.d[1];
    }

    IPluginV2DynamicExt *clone() const noexcept override
    {
      return new NMSRotatedPlugin(_nms_thresh, _detections_per_im, _count);
    }
  ...
  class NMSRotatedPluginCreator : public IPluginCreator
  {
  public:
    ...
    {
      return new NMSRotatedPlugin(serialData, serialLength);
    }
  ...
  REGISTER_TENSORRT_PLUGIN(NMSRotatedPluginCreator);
