...
#define PLUGIN_NAME "BytetrackDecodeRotate"
...
namespace rapid
{
  ...
  public:
    // DecodeRotatePlugin(float score_thresh, int top_n, int stride)
                                  ...
                                  int nbInputs, IExprBuilder &exprBuilder) noexcept override
    {
      ...
      if (outputIndex == 1)
      {
        output.d[1] = exprBuilder.constant(_top_n * 5);
      }
      else
      {
                            ...
                            int nbInputs, const PluginTensorDesc *outputs, int nbOutputs) const noexcept override
    {
      if (size < 0)
      {
        size = cuda::decode_rotate(inputs->dims.d[0], nullptr, nullptr,
                ...
                void *const *outputs, void *workspace, cudaStream_t stream) noexcept override
    {
      ...
      return cuda::decode_rotate(inputDesc->dims.d[0], inputs, outputs,
                          _top_n, _f_width, _f_height, _score_thresh, _stride,
    ...
    {
      return new DecodeRotatePlugin(_score_thresh, _top_n, _stride, _f_width, _f_height);
    }
