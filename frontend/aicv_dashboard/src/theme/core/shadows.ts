...
const LIGHT_MODE = grey['500Channel'];
const DARK_MODE = common.blackChannel;

const createShadow = (color: string): Shadows => {
  const color1 = varAlpha(color, 0.2);
  const color2 = varAlpha(color, 0.14);
  const color3 = varAlpha(color, 0.12);
